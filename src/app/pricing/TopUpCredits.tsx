...
export default function TopUpCredits() {
  ...
  return (
    <div className="group relative flex h-full flex-col rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.07] hover:shadow-black/50">
      ...
      <div className="mt-1 text-xs font-medium uppercase tracking-widest text-[var(--muted)]">
        One-time purchase
      </div>
      <div className="mt-6 text-4xl font-bold tracking-tight text-white">$5</div>
      <div className="mt-1 text-xs font-medium uppercase tracking-widest text-[var(--muted)]">
        per 100 credits
      </div>
      ...
      <div className="mt-8">
        ...
        <div className="relative mt-3">
          <select
            ...
            className="w-full appearance-none rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white transition-colors focus:border-[var(--accent)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)]"
          >
      ...
      <div className="mt-4 text-sm text-[var(--muted)]">
        Total: <span className="font-medium text-white">${price.toFixed(0)}</span>
      </div>

      <div className="mt-auto pt-8">
        <Link
          href="/pricing?topup=1"
          className="inline-flex w-full items-center justify-center rounded-full border border-white/10 bg-white/5 px-6 py-4 text-sm font-semibold text-white transition-all duration-200 hover:bg-white/10 hover:scale-[1.02] active:scale-[0.98]"
        >
          Add credits
        </Link>
      </div>
